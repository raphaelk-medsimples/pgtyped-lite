...
const getNotificationsIR: any = {"name":"GetNotifications","usedParamSet":{"userId":true,"date":true},"params":[{"name":"userId","required":false,"transform":{"type":"scalar"},"locs":[{"a":47,"b":53}]},{"name":"date","required":true,"transform":{"type":"scalar"},"locs":[{"a":73,"b":78}]}],"statement":"SELECT *\n  FROM notifications\n WHERE user_id = :userId\n AND created_at > :date!"};
...
const thresholdFrogsIR: any = {"name":"ThresholdFrogs","usedParamSet":{"numFrogs":true},"params":[{"name":"numFrogs","required":true,"transform":{"type":"scalar"},"locs":[{"a":143,"b":152}]}],"statement":"SELECT u.user_name, n.payload, n.type\nFROM notifications n\nINNER JOIN users u on n.user_id = u.id\nWHERE CAST (n.payload->'num_frogs' AS int) > :numFrogs!"};
...
export default (db: IDatabaseConnection) => ({
  sendNotifications: (params: ISendNotificationsParams) => sendNotifications.run(params, db),
  getNotifications: (params: IGetNotificationsParams) => getNotifications.run(params, db),
  thresholdFrogs: (params: IThresholdFrogsParams) => thresholdFrogs.run(params, db),
});
